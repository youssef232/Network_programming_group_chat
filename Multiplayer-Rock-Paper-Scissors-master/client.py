...
class Button:
    """
    Class for buttons
    it takes text, x, y, and colour as parameters
    ...
    def draw(self, win):
        """
        to render the button on the screen
        :param win: win parameter, which represents the Pygame window surface
        :return: None "it just renders the button on the screen aka void function"
        """
...
def re_draw_window(win, game, player):
    """
    The purpose of the re_draw_window function is to update and
    redraw the Pygame window based on the current state of the game and the player.
    :param win:  (the Pygame window surface)
    :param game: the current game object
    :param player: player identifier (0 or 1)
    :return:
    """
    win.fill((0, 13, 26))
...
def main():
    ...
    while run:
        ...
        # render the results of the game
        if game.both_went():
